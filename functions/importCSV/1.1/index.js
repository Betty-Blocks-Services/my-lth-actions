...
const importCsv = async ({
  ...
  try {
    ...
    if (deduplicate && importRecords && importRecords.length > 0) {
      if (uniqueRecordColumnType === "DECIMAL") {
        // decimal does not work with IN FILTER
        importRecords.forEach((item) => {
          if (item[uniqueRecordColumnName])
            whereFilterData += `{${uniqueRecordIdentifier.value} : {eq:\"${item[uniqueRecordColumnName]}\"}},`;
        });
        whereFilter = `{ _or: [${whereFilterData}]}`;
      } else {
        importRecords.forEach((item) => {
          if (item[uniqueRecordColumnName])
