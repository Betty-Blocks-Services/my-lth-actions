          ...
          if (mapping.key === formatMapping.key && record[mapping.key]) {
            switch (formatMapping.value.toLowerCase().trim()) {
              case "decimal":
                ...
                const sanitizedDecimalValue = record[mapping.key]
                  .toString()
                  .replace(",", ".");
                if (!isNaN(parseFloat(sanitizedDecimalValue))) {
                  if (sanitizedDecimalValue.indexOf(".") === -1)
                    record[mapping.key] = sanitizedDecimalValue + ".00";
                  else {
                    record[mapping.key] = parseFloat(sanitizedDecimalValue)
                      .toFixed(2)
                      .toString();
                  }
                } else {
                ...
                break;
              case "number":
                if (isNaN(parseFloat(record[mapping.key]))) {
                  record[mapping.key] = "";
                }
                break;
              case "checkbox":
